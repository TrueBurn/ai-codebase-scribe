...
class CodebaseAnalyzer:
    ...
    def __init__(self, repo_path: Path, config: ScribeConfig):
        ...
        if (
            hasattr(self.config_obj, "github_repo_id")
            and self.config_obj.github_repo_id
        ):
        ...
        # Initialize cache with ScribeConfig
        self.cache = CacheManager(
            enabled=not self.config_obj.no_cache,
        ...
        try:
            ...
            for gitignore_path in self.repo_path.rglob(".gitignore"):
                if gitignore_path.is_file():
                    ...
                    _ = gitignore_path.parent.relative_to(self.repo_path)
                    ignore_func = parse_gitignore(
                        gitignore_path, base_dir=str(self.repo_path)
                    )
                    ...
                    if self.debug:
                        try:
                            with open(gitignore_path, "r") as f:
                                ...
                                logging.debug(
                                    f"Sample patterns from {gitignore_path}: {patterns}"
                                )
                ...
                try:
                    ...
                    for fn in ignore_funcs:
                        if fn(path_str):
                            if self.debug:
                                logging.debug(
                                    f"Ignoring {path_str} due to gitignore rule"
                                )
                            return True
                    return False
                except ValueError:
    ...
    def is_binary(self, file_path: Path) -> bool:
        ...
        try:
            ...
            # Handle regex memory errors or other unexpected errors
            error_str = str(e)
            if "regex error" in error_str and "failed to get memory" in error_str:
                logging.warning(
                    f"Regex memory error when checking file type for {file_path}. Falling back to extension-based check."
                )
                # For C++ files that might trigger the regex memory error
                ext = file_path.suffix.lower()
                if ext in [".cpp", ".cc", ".cxx", ".h", ".hpp", ".hxx"]:
                    return False  # Treat C++ files as text
                # Fall back to simple binary check
                return self._is_binary(file_path)
            else:
                # For other unexpected errors, log and fall back
                logging.error(f"Unexpected error checking if file is binary: {e}")
                return self._is_binary(file_path)
    ...
    def should_include_file(self, file_path: Path) -> bool:
        ...
        if file_path.name in self.SPECIAL_FILES or any(
            file_path.name.endswith(f) for f in self.SPECIAL_FILES
        ):
            if self.debug:
        ...
        if any(
            part.startswith(".") and part != ".gitignore" for part in file_path.parts
        ):
            if self.debug:
        ...
        for pattern in self.blacklist_patterns:
            if re.search(pattern, str(file_path)):
                if self.debug:
                    logging.debug(
                        f"Excluding file matching blacklist pattern '{pattern}': {file_path}"
                    )
                return False
    ...
    def analyze_repository(self, show_progress: bool = False) -> Dict[str, FileInfo]:
        ...
        try:
            ...
            if test_mode:
                ...
                if self.debug:
                    logging.debug(
                        f"Test mode enabled, limiting to {len(all_files)} files"
                    )
            ...
            if show_progress:
                try:
                    ...
                    with progress_tracker.progress_bar(
                        desc="Analyzing repository", total=len(all_files), unit="files"
                    ) as pbar:
        ...
        try:
            for file_path in self.repo_path.rglob("*"):
                ...
                if not self.should_include_file(rel_path):
                    if self.debug:
                        logging.debug(
                            f"Excluding file due to inclusion rules: {rel_path}"
                        )
                    continue
    ...
    def _analyze_file(self, file_path: Path) -> FileInfo:
        ...
        try:
            ...
            return FileInfo(path=file_path.relative_to(self.repo_path), is_binary=False)
    ...
    def read_file(self, file_path: Path) -> str:
        ...
        try:
            ...
            try:
                ...
                if self.debug:
                    logging.error(
                        f"Error reading file {file_path} with latin-1 encoding: {e}"
                    )
                raise
        except Exception as e:
            if self.debug:
    ...
    def check_markdown_headers(self, content: str) -> list[str]:
        ...
        for i, line in enumerate(lines):
            ...
            if level > 5:
                issues.append(
                    f"Header on line {i+1} has too many # symbols (level {level})"
                )

            # Check for space after #
            if not re.match(r"^#+\s", line):
                issues.append(
                    f"Header on line {i+1} is missing a space after # symbols"
                )
            ...
            if header_text and not header_text[0].isupper():
                issues.append(
                    f"Header on line {i+1} should start with a capital letter"
                )
    ...
    def build_dependency_graph(self) -> nx.DiGraph:
        ...
        if self.debug:
            logging.debug(
                f"Dependency graph has {len(self.graph.nodes())} nodes and {len(self.graph.edges())} edges"
            )
