...
class ConfigManager:
    ...
    def _apply_env_overrides(self, config: dict) -> dict:
        ...
        if os.getenv(ENV_AWS_VERIFY_SSL):
            config["bedrock"]["verify_ssl"] = os.getenv(ENV_AWS_VERIFY_SSL).lower() in (
                "true",
                "1",
                "yes",
            )
        ...
        if os.getenv(ENV_CACHE_ENABLED):
            config["cache"]["enabled"] = os.getenv(ENV_CACHE_ENABLED).lower() in (
                "true",
                "1",
                "yes",
            )
    ...
    def _validate_config(self, config: dict) -> None:
        ...
        if config.get("llm_provider") not in ["ollama", "bedrock"]:
            raise ConfigValidationError(
                f"Invalid LLM provider: {config.get('llm_provider')}. Must be 'ollama' or 'bedrock'."
            )
        ...
        if (
            not isinstance(ollama_config.get("max_tokens", 0), int)
            or ollama_config.get("max_tokens", 0) <= 0
        ):
        ...
        if (
            not isinstance(bedrock_config.get("max_tokens", 0), int)
            or bedrock_config.get("max_tokens", 0) <= 0
        ):
            raise ConfigValidationError(
                "Bedrock max_tokens must be a positive integer."
            )
        ...
        if cache_config.get("location") not in ["repo", "home"]:
            raise ConfigValidationError(
                f"Invalid cache location: {cache_config.get('location')}. Must be 'repo' or 'home'."
            )

        if cache_config.get("hash_algorithm") not in ["md5", "sha1", "sha256"]:
            raise ConfigValidationError(
                f"Invalid hash algorithm: {cache_config.get('hash_algorithm')}. Must be 'md5', 'sha1', or 'sha256'."
            )
    ...
    def get_template(
        self, category: str, name: str, context: Optional[Dict[str, Any]] = None
    ) -> str:
        """
        ...
        """
        if (
            category not in self.config["templates"]
            or name not in self.config["templates"][category]
        ):
