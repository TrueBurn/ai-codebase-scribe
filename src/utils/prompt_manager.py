...
""".strip(),
                ...
                for name, content in config["templates"].items():
                    ...
                    if (
                        ...
                        self.template_versions[name] = (
                            f"custom-{datetime.now().strftime('%Y%m%d')}"
                        )

            self.last_loaded = datetime.now().isoformat()
            logging.info(
                f"Loaded {len(config.get('templates', {}))} custom templates from {config_path}"
            )
    ...
    def _validate_all_templates(self):
        ...
        for name, template in self.templates.items():
            if name in self.TEMPLATE_REQUIREMENTS:
                missing = self._validate_template(
                    template, self.TEMPLATE_REQUIREMENTS[name]
                )
                if missing:
                    logging.warning(
                        f"Template '{name}' is missing required placeholders: {', '.join(missing)}"
                    )

    def _validate_template(
        self, template: str, required_placeholders: List[str]
    ) -> List[str]:
        """
    ...
    def get_template(
        ...
        if template_name not in self.templates:
            ...
            return "Please analyze the following content: {content}"
        ...
        if context:
            ...
            if template_name in self.TEMPLATE_REQUIREMENTS:
                missing = [
                    p
                    for p in self.TEMPLATE_REQUIREMENTS[template_name]
                    if p not in context and p not in self._get_default_keys()
                ]
                if missing:
                    logging.warning(
                        f"Context for template '{template_name}' is missing required keys: {', '.join(missing)}"
                    )
    ...
    def add_template(
        self, name: str, content: str, version: Optional[str] = None
    ) -> bool:
        """
        ...
        self.templates[name] = content
        self.template_versions[name] = (
            version or f"custom-{datetime.now().strftime('%Y%m%d')}"
        )
        ...
        if name in self.TEMPLATE_REQUIREMENTS:
            ...
            if missing:
                logging.warning(
                    f"New template '{name}' is missing required placeholders: {', '.join(missing)}"
                )
