...
class CacheConfig:
    ...
    global_directory: str = (
        "readme_generator_cache"  # Global cache directory name (no dot to make it visible)
    )
...
class LargeRepoConfig:
    ...
    create_pr_on_batch: bool = (
        False  # IMPORTANT: False for open-source (requires GitHub token)
    )
...
class ScribeConfig:
    ...
    troubleshooting: TroubleshootingConfig = field(
        default_factory=TroubleshootingConfig
    )
    ...
    def from_dict(cls, config_dict: Dict[str, Any]) -> "ScribeConfig":
        ...
        if "blacklist" in config_dict:
            ...
            config.blacklist = BlacklistConfig(
                ...
                path_patterns=blacklist_dict.get(
        ...
        if "cache" in config_dict:
            ...
            logging.debug(
                f"Cache location from dictionary: {cache_dict.get('location', 'default')}"
            )
            ...
            config.cache = CacheConfig(
                ...
                global_directory=cache_dict.get(
        ...
        if "large_repo" in config_dict:
            ...
            config.large_repo = LargeRepoConfig(
                **{
                    k: v
                    for k, v in large_repo_data.items()
        ...
        if "bedrock" in config_dict:
            ...
            config.bedrock = BedrockConfig(
                ...
                max_output_tokens_architecture=bedrock_dict.get(
                    "max_output_tokens_architecture", 32768
                ),
                max_output_tokens_persistence=bedrock_dict.get(
                ...
                extended_context_enabled=bedrock_dict.get(
        ...
        if "templates" in config_dict:
            ...
            config.templates = TemplatesConfig(prompts=prompts_config, docs=docs_config)
        ...
        if "persistence" in config_dict:
                ...
                if k == "supported_technologies" and isinstance(v, dict):
                    p_config.supported_technologies = PersistenceSupportedTechnologiesConfig(
                        **{
                            ...
                            if kk
                            in PersistenceSupportedTechnologiesConfig.__dataclass_fields__
                        }
                    )
            ...
            logging.debug(
                f"Loaded persistence configuration - enabled: {config.persistence.enabled}"
            )
        ...
        if "installation" in config_dict:
            config.installation = InstallationConfig(
                **{
                    k: v
                    for k, v in config_dict["installation"].items()
                    if k in InstallationConfig.__dataclass_fields__
                }
            )
            logging.debug(
                f"Loaded installation configuration - enabled: {config.installation.enabled}"
            )
        ...
        if "usage" in config_dict:
            config.usage = UsageConfig(
                **{
                    k: v
                    for k, v in config_dict["usage"].items()
                    if k in UsageConfig.__dataclass_fields__
                }
            )
            logging.debug(
                f"Loaded usage configuration - enabled: {config.usage.enabled}"
            )
        ...
        if "troubleshooting" in config_dict:
            config.troubleshooting = TroubleshootingConfig(
                **{
                    k: v
                    for k, v in config_dict["troubleshooting"].items()
                    if k in TroubleshootingConfig.__dataclass_fields__
                }
            )
            logging.debug(
                f"Loaded troubleshooting configuration - enabled: {config.troubleshooting.enabled}"
            )
        ...
        if "contributing" in config_dict:
            config.contributing = ContributingConfig(
                **{
                    k: v
                    for k, v in config_dict["contributing"].items()
                    if k in ContributingConfig.__dataclass_fields__
                }
            )
            logging.debug(
                f"Loaded contributing configuration - enabled: {config.contributing.enabled}"
            )
        ...
        if "readme_refactor" in config_dict:
            config.readme_refactor = ReadmeRefactorConfig(
                **{
                    k: v
                    for k, v in config_dict["readme_refactor"].items()
                    if k in ReadmeRefactorConfig.__dataclass_fields__
                }
            )
            logging.debug(
                f"Loaded readme_refactor configuration - enabled: {config.readme_refactor.enabled}"
            )
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "templates": {
                ...
                "docs": {"readme": self.templates.docs.readme},
            },
        }
