...
class ProgressTracker:
    ...
    def get_instance(cls, repo_path: Optional[Path] = None) -> "ProgressTracker":
        ...
        if cls._instance is None:
            if repo_path is None:
                raise ValueError(
                    "repo_path is required when creating the first instance"
                )
    ...
    def __init__(self, repo_path: Path):
        ...
        self.main_pbar = tqdm(
            desc="Overall Progress", unit="task", position=0, leave=True
        )
    ...
    def update_task(
        self, name: str, advance: int = 1, message: Optional[str] = None
    ) -> None:
        """Update task progress.
    ...
    def complete_task(
        self, name: str, success: bool = True, message: Optional[str] = None
    ) -> None:
        ...
        try:
            if name in self.tasks:
                ...
                if task.progress_bar:
                    try:
                        task.progress_bar.update(
                            task.progress_bar.total - task.progress_bar.n
                        )
                        if message:
                            task.progress_bar.set_postfix_str(message)
                        ...
                        self.logger.warning(
                            f"Error closing progress bar for task '{name}': {pb_error}"
                        )
                ...
                try:
                    ...
                    self.logger.warning(
                        f"Error updating main progress bar: {mpb_error}"
                    )
                ...
                self.logger.log(
                    log_level,
                    f"Completed task: {name}{duration_str} - {message if message else 'Done'}",
                )
            else:
    ...
    def skip_task(self, name: str, reason: str) -> None:
        ...
        try:
            ...
            try:
                ...
                self.logger.warning(
                    f"Error with progress bar for skipped task '{name}': {pb_error}"
                )
                if progress_bar in self._active_progress_bars:
                    self._active_progress_bars.remove(progress_bar)
    ...
    def summary(self) -> None:
        ...
        try:
            ...
            if (
                hasattr(self, "main_pbar")
                and self.main_pbar in self._active_progress_bars
            ):
                try:
            ...
            self.logger.info(
                f"Tasks: {len(self.tasks)} total, {successful} successful, {failed} failed, {skipped} skipped"
            )
