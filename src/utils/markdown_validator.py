...
class MarkdownValidator:
    ...
    async def validate_with_link_checking(
        self, repo_path: Path, base_path: Path = None
    ...
    def _check_code_blocks(self) -> List[ValidationIssue]:
        ...
        if in_code_block:
            issues.append(
                ValidationIssue(
                    len(self.lines), "Unclosed code block", "error", "Add closing ```"
                )
            )
        ...
        for i, line in enumerate(self.lines):
            ...
            if stripped.startswith("|") and stripped.endswith("|"):
                ...
                if not in_table:
                    ...
                    is_separator = all(
                        cell.strip() == "" or all(c in "-:|" for c in cell)
                        for cell in cells
                    )
    ...
    def fix_common_issues(self) -> str:
        ...
        for i, line in enumerate(fixed_lines):
            ...
            if list_match:
                ...
                if not in_list:
                    ...
                    if current_indent > list_indent_base:
                        desired_indent = (
                            ...
                            + (
                                (
                                    (current_indent - list_indent_base)
                                    + (LIST_INDENT_SPACES - 1)
                                )
                                // LIST_INDENT_SPACES
                            )
                            * LIST_INDENT_SPACES
                ...
                if i + 1 < len(fixed_lines) and not re.match(
                    r"^\s*[*+-]\s", fixed_lines[i + 1]
                ):
                    in_list = False
            else:
