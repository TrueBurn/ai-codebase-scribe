...
class TokenCounter:
    ...
    def __init__(
        ...
        self.buffer_percentage = (
            ...
            else self.DEFAULT_BUFFER_PERCENTAGE
        )
        self.truncate_buffer = (
            truncate_buffer
            if truncate_buffer is not None
            else self.DEFAULT_TRUNCATE_BUFFER
        )
        self.chunk_buffer = (
            chunk_buffer if chunk_buffer is not None else self.DEFAULT_CHUNK_BUFFER
        )
        self.oversized_target = (
            oversized_target
            if oversized_target is not None
            else self.DEFAULT_OVERSIZED_TARGET
        )
        ...
        try:
            ...
            if debug:
                self.logger.debug(
                    f"Using encoding {encoding_name} for model {model_name}"
                )

        except (Exception, ImportError) as e:
            self.logger.warning(
                f"Failed to load encoding {encoding_name}: {str(e)}. Falling back to cl100k_base."
            )
            try:
                ...
                self.logger.error(
                    f"Failed to load fallback encoding: {str(e2)}. Token counting will be approximate."
                )
    ...
    def _load_model_limits_from_config(self, config_path: str) -> None:
        ...
        try:
            ...
            if config_file.exists():
                ...
                if "model_limits" in config_data and isinstance(
                    config_data["model_limits"], dict
                ):
            ...
            self.logger.warning(
                f"Failed to load model limits from {config_path}: {str(e)}"
            )
    ...
    def chunk_text(
        self, text: str, chunk_size: Optional[int] = None, overlap: int = 100
    ) -> List[str]:
    ...
    def handle_oversized_input(
        self, text: str, target_percentage: Optional[float] = None
    ) -> str:
        ...
        if self.debug:
            self.logger.debug(
                f"Reducing input from {current_tokens} to {target_tokens} tokens"
            )
        ...
        if current_tokens <= target_tokens:
            if self.debug:
                self.logger.debug(
                    f"Reduced to {current_tokens} tokens by removing duplicate empty lines"
                )
            return text
        ...
        if "Project Structure:" in text:
            ...
            if current_tokens <= target_tokens:
                if self.debug:
                    self.logger.debug(
                        f"Reduced to {current_tokens} tokens by simplifying project structure"
                    )
                return text
