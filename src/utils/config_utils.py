...
def update_config_with_args(config: ScribeConfig, args: Any) -> ScribeConfig:
    ...
    # Debug logging
    logging.debug("Updating config with args")
...
def apply_env_overrides(config: ScribeConfig) -> ScribeConfig:
    ...
    if os.getenv(ENV_LLM_PROVIDER):
        ...
        logging.debug(
            f"Applied environment override for LLM provider: {new_config.llm_provider}"
        )
    ...
    if os.getenv(ENV_AWS_REGION):
        ...
        logging.debug(
            f"Applied environment override for AWS region: {new_config.bedrock.region}"
        )

    if os.getenv(ENV_AWS_BEDROCK_MODEL_ID):
        new_config.bedrock.model_id = os.getenv(ENV_AWS_BEDROCK_MODEL_ID)
        logging.debug(
            f"Applied environment override for Bedrock model ID: {new_config.bedrock.model_id}"
        )

    if os.getenv(ENV_AWS_VERIFY_SSL):
        new_config.bedrock.verify_ssl = os.getenv(ENV_AWS_VERIFY_SSL).lower() in (
            "true",
            "1",
            "yes",
        )
        logging.debug(
            f"Applied environment override for verify SSL: {new_config.bedrock.verify_ssl}"
        )

    if os.getenv(ENV_CACHE_ENABLED):
        new_config.cache.enabled = os.getenv(ENV_CACHE_ENABLED).lower() in (
            "true",
            "1",
            "yes",
        )
        logging.debug(
            f"Applied environment override for cache enabled: {new_config.cache.enabled}"
        )
    ...
    if os.getenv(ENV_CACHE_HASH_ALGORITHM):
        ...
        if hash_algo in ["md5", "sha1", "sha256"]:
            ...
            logging.debug(
                f"Applied environment override for hash algorithm: {new_config.cache.hash_algorithm}"
            )

    if os.getenv(ENV_CACHE_GLOBAL_DIRECTORY):
        new_config.cache.global_directory = os.getenv(ENV_CACHE_GLOBAL_DIRECTORY)
        logging.debug(
            f"Applied environment override for global directory: {new_config.cache.global_directory}"
        )
