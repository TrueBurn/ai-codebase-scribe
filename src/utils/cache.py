...
class CacheManager:
    ...
    def get_repo_cache_dir(self, repo_path: Optional[Path] = None) -> Path:
        ...
        if not repo_path:
            if self.debug:
                print(
                    "Warning: No repository path provided, using default cache directory"
                )
        ...
        if cache_location == "home":
            ...
            global_cache_dir = (
                ...
                if hasattr(self, "global_cache_dir")
                else self.DEFAULT_GLOBAL_CACHE_DIR
            )
            ...
            if self.repo_identifier:
                ...
                if self.debug:
                    print(
                        f"Using home directory for cache: {home_cache_dir / safe_dir}"
                    )
            ...
            cache_dir_name = (
                self.cache_dir_name if hasattr(self, "cache_dir_name") else ".cache"
            )
    ...
    def clear_repo_cache(self):
        ...
        try:
            ...
            print("Cache cleared for repository")
    ...
    def get_cached_summary(self, file_path: Path) -> Optional[str]:
        ...
        try:
            ...
            result = conn.execute(
                "SELECT summary FROM file_cache WHERE file_path = ?", (cache_key,)
            ).fetchone()
            ...
            if self._repo_path:
                ...
                try:
                    ...
                    if self.repo_identifier:
                        ...
                        if self.debug:
                            print(
                                f"Created stable cache key with repo identifier: {cache_key}"
                            )
    ...
    def _is_file_modified(self, file_path: Path) -> bool:
        ...
        try:
            ...
            result = conn.execute(
                "SELECT content_hash FROM file_cache WHERE file_path = ?", (cache_key,)
            ).fetchone()
        ...
        try:
            ...
            if cache_location == "home":
                ...
                if (
                    repo_path
                    and config_obj
                    and hasattr(config_obj, "github_repo_id")
                    and config_obj.github_repo_id
                ):
