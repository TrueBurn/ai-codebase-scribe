...
class ReadabilityScorer:
    ...
    def analyze_text(self, text: str, section_name: str) -> ReadabilityMetrics:
        ...
        overall = (
            (100 - min(flesch_reading_ease, 100))
            * 0.4  # Invert so higher = more complex
    ...
    def get_recommendations(self, section_name: str) -> List[str]:
        ...
        if section_name not in self.scores:
            raise KeyError(
                f"No analysis found for section '{section_name}'. Run analyze_text first."
            )
        ...
        if (
            metrics["complex_word_percentage"]
            > self.thresholds["complex_word_percentage"]
        ):
    ...
    def is_section_readable(
        self, section_name: str, target_level: str = "medium"
    ) -> bool:
        """
        ...
        if target_level not in self.GRADE_LEVELS:
            raise KeyError(
                f"Invalid target level '{target_level}'. Must be one of: {', '.join(self.GRADE_LEVELS.keys())}"
            )
        ...
        return (
            metrics["flesch_kincaid_grade"] <= target_grade
            and metrics["gunning_fog"] <= target_grade + 2
        )
