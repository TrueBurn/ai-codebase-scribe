...
async def clone_github_repository(url: str, github_token: Optional[str] = None) -> Path:
    ...
    try:
        ...
        logging.warning(
            f"Could not create directory in user home, falling back to system temp: {e}"
        )
...
async def cleanup_temp_dir(temp_dir: Path) -> None:
    ...
    try:
            ...
            for root, dirs, files in os.walk(temp_dir):
                for dir_name in dirs:
                    try:
                        ...
                        logging.debug(
                            f"Could not change permissions on directory {dir_path}: {e}"
                        )
                for file_name in files:
                    try:
                        ...
                        logging.debug(
                            f"Could not change permissions on file {file_path}: {e}"
                        )
        ...
        try:
            ...
            logging.warning(
                f"Standard cleanup failed, trying alternative approach: {e}"
            )
...
def find_existing_pr(
    github_url: str, token: str, branch_name: str
    ...
    try:
        ...
        logging.error(
            "PyGithub package not installed. Install with: pip install PyGithub"
        )
        return None
    ...
    try:
        ...
        try:
            ...
            if "Not Found" in str(e):
                logging.info(
                    f"Branch '{branch_name}' doesn't exist in remote repository"
                )
...
def create_git_branch(repo_dir: Path, branch_name: str) -> bool:
    ...
    try:
        ...
        branch_exists = branch_name in [
            ref.name for ref in repo.references if isinstance(ref, git.refs.Reference)
        ]
        ...
        raise GitOperationError(
            f"Failed to create or checkout branch '{branch_name}': {str(e)}"
        ) from e


def commit_documentation_changes(
    repo_dir: Path, files: list = None, message: str = "Add generated documentation"
) -> bool:
    ...
    try:
        ...
        logging.info(
            f"Committed changes with message: '{message}' (commit: {commit.hexsha[:8]})"
        )
        return True
        ...
        raise GitOperationError(
            f"Failed to commit documentation changes: {str(e)}"
        ) from e


async def push_branch_to_remote(
    repo_dir: Path, branch_name: str, token: str, github_url: str
) -> bool:
    ...
    try:
        ...
        push_info = remote.push(
            branch_name, force=True
        )  # Use force=True to ensure we can update existing branch
        ...
        raise GitOperationError(
            f"Failed to push branch '{branch_name}' to remote: {str(e)}"
        ) from e
    finally:
...
def extract_repo_info(github_url: str) -> Tuple[str, str]:
    ...
    if len(path_parts) < 2:
        raise ValueError(
            f"URL does not contain both owner and repository: {github_url}"
        )
    ...
    logging.debug(
        f"Extracted repo info from {github_url}: owner={owner}, repo={repo_name}"
    )
