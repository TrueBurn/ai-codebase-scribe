...
class LinkValidator:
    ...
    def __init__(
        ...
        self.anchor_map: Dict[str, Set[str]] = (
            {}
        )  # Map of document paths to sets of anchors
        ...
        if cache_key not in self.checked_urls:
            ...
            if time.time() - cache_entry["timestamp"] < 3600:  # 1 hour cache
                ...
                for issue in cache_entry["issues"]:
                    ...
                    self.issues.append(
                        LinkIssue(
                            issue.link, line_number, issue.message, issue.severity
                        )
                    )
            else:
    ...
    def _validate_internal_link(
        self, url: str, line_number: int, base_path: Path
    ) -> None:
        ...
        try:
            _ = target_path.relative_to(self.repo_path)
        except ValueError:
            self.issues.append(
                LinkIssue(url, line_number, "Link points outside repository", "error")
            )
            return

        if not target_path.exists():
            self.issues.append(
                LinkIssue(url, line_number, "Referenced file does not exist", "error")
            )
            return
        ...
        if anchor_part:
            ...
            if (
                target_doc_path in self.anchor_map
                and anchor_part not in self.anchor_map[target_doc_path]
            ):
                self.issues.append(
                    LinkIssue(
                        url,
