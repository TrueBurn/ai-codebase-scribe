...
class OllamaClient(BaseLLMClient):
        ...
        try:
            ...
            if not self.available_models:
                raise OllamaClientError(
                    "No models available in Ollama instance. Please ensure Ollama is running and has models installed."
                )
    ...
    def init_token_counter(self) -> None:
        ...
        self.token_counter = TokenCounter(
            model_name=self.selected_model, debug=self.debug
        )
    ...
    async def generate_summary(
        ...
        try:
            ...
            # Check token count
            will_exceed, token_count = self.token_counter.will_exceed_limit(
                prompt, self.selected_model
            )

            if will_exceed:
                if self.debug:
                    print(
                        f"Content exceeds token limit ({token_count} tokens). Truncating..."
                    )
                prompt = self.token_counter.truncate_text(prompt)

                # Re-check after truncation
                _, new_token_count = self.token_counter.will_exceed_limit(
                    prompt, self.selected_model
                )
                if self.debug:
        ...
        while True:
            ...
            try:
                ...
                if 1 <= selection <= len(self.available_models):
                    ...
                    print(
                        f"Please choose a number between 1 and {len(self.available_models)}"
                    )
                    print("\n" + "-" * 50 + "\n")
            except ValueError:
...
Focus on detecting relationships between tables. Return only valid JSON."""
            ...
            try:
                ...
                logging.error(f"JSON parsing error during Ollama aggregation: {e}")
                return self._simple_aggregation_fallback(individual_analyses)

        except Exception as e:
            logging.error(f"Error during Ollama migration analysis aggregation: {e}")
            return self._simple_aggregation_fallback(individual_analyses)
