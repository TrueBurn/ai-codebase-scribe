...
def format_project_structure(
    ...
    try:
        ...
        use_compression = (
            force_compression if force_compression is not None else len(file_paths) > 50
        )
        ...
        if use_compression:
            ...
            for path in compressed_paths:
                ...
                for i, part in enumerate(
                    parts[:-1]
                ):  # All parts except the last one (filename)
        ...
        logging.debug(
            f"Project structure formatted successfully with {len(structure_lines)} lines"
        )
...
def find_common_dependencies(
    file_manifest: Dict[str, Dict], debug: bool = False
) -> str:
    """
    ...
    try:
        ...
        for path, info in file_manifest.items():
            ...
            is_binary = (
                info.get("is_binary", False)
                if isinstance(info, dict)
                else getattr(info, "is_binary", False)
            )
            if path_str.endswith("package.json") and not is_binary:
                ...
                try:
                    ...
                    content = (
                        info.get("content", "")
                        if isinstance(info, dict)
                        else getattr(info, "content", "")
                    )
                    if content:
                        # Check if content is a string
                        if not isinstance(content, str):
                            logging.warning(
                                f"Content for {path} is not a string, but {type(content)}"
                            )
                            continue
                        ...
                        if (
                            ...
                            or '"devDependencies"' in content
                        ):
                            # Try to parse as JSON first (most reliable)
                            try:
                                package_data = json.loads(content)
                                # Process dependencies
                                if "dependencies" in package_data and isinstance(
                                    package_data["dependencies"], dict
                                ):
                                    for dep_name, dep_version in package_data[
                                        "dependencies"
                                    ].items():
                                        if isinstance(dep_version, str):
                                            dependencies[
                                                f"npm:{dep_name}@{dep_version}"
                                            ] += 1
                                # Process devDependencies
                                if "devDependencies" in package_data and isinstance(
                                    ...
                                    for dep_name, dep_version in package_data[
                                        ...
                                        if isinstance(dep_version, str):
                                            dependencies[
                                                f"npm-dev:{dep_name}@{dep_version}"
                                            ] += 1
                            except json.JSONDecodeError:
                                # Fallback to regex if JSON parsing fails
                                # Extract dependencies section
                                deps_match = re.search(
                                    r'"dependencies"\s*:\s*{([^}]+)}', content
                                )
                                if deps_match:
            ...
            if path_str.endswith("packages.config") and not is_binary:
                ...
                try:
                    ...
                    if content:
                        ...
                        for match in re.finditer(
                            r'<package\s+id="([^"]+)"\s+version="([^"]+)"', content
                        ):
                            package_name = match.group(1)
                            version = match.group(2)
                            dependencies[f"nuget:{package_name}@{version}"] += 1
                ...
                try:
                    ...
                    if content:
                        ...
                        for match in re.finditer(
                            ...
                            if artifact:
                                dependencies[
                                    f"gradle:{group}:{artifact}@{version}"
                                ] += 1
                except Exception as e:
            ...
            if path_str.endswith("build.gradle.kts") and not is_binary:
                ...
                try:
                    ...
                    if content:
                        ...
                        # Extract implementation/compile dependencies
                        for match in re.finditer(
                            r'(implementation|compile)\([\'"](.*?)[\'"]', content
                        ):
                            _ = match.group(1)
        ...
        logging.debug(
            f"Processed {package_json_count} package.json, {requirements_txt_count} requirements.txt, "
...
def identify_key_components(
    ...
    try:
        ...
        logging.debug(
            f"Identified {min(len(sorted_dirs), max_components)} key components out of {len(sorted_dirs)} directories"
        )
        return result
    except Exception as e:
...
def get_default_order(
    core_files: Dict[str, Dict], resource_files: Dict[str, Dict]
) -> List[str]:
    """
    ...
    config_files = [
        p for p in core_files if p.endswith((".json", ".config", ".settings"))
    ]
    ...
    logging.debug(
        f"Default order: {len(config_files)} config files, {len(other_files)} other files, {len(resource_list)} resource files"
    )
...
def prepare_file_order_data(
    project_files: Dict[str, Dict],
    debug: bool = False,
    vendor_patterns: List[str] = DEFAULT_VENDOR_PATTERNS,
    ...
    for path, info in project_files.items():
        is_vendor = any(
            re.search(pattern, path, re.IGNORECASE) for pattern in vendor_patterns
        )
        if is_vendor:
    ...
    if debug:
        print(
            f"Filtered {len(core_files)} core files and {len(resource_files)} resource files"
        )
    logging.info(
        f"File filtering: {len(core_files)} core files and {len(resource_files)} resource files"
    )
    ...
    for path, info in core_files.items():
        ...
        files_info[path] = {
            "type": (
                info.file_type
                if hasattr(info, "file_type") and info.file_type
                else Path(path).suffix
            ),
...
def process_file_order_response(
    content: str,
    core_files: Dict[str, Dict],
    resource_files: Dict[str, Dict],
    debug: bool = False,
        ...
        if isinstance(result, dict) and "file_order" in result:
            ...
            if valid_paths:
                if debug and "reasoning" in result:
                    ...
                    print(
                        f"Ordering reasoning: {reasoning[:100]}..."
                ...
                logging.debug(
                    f"Successfully extracted file order from JSON response with {len(valid_paths)} valid paths"
                )
                return full_order
            else:
