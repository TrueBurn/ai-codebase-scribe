...
def should_enhance_existing_readme(repo_path: Path, config: ScribeConfig) -> bool:
    ...
    try:
        ...
        return (
            len(existing_content.strip().split("\n"))
...
async def enhance_existing_readme(
    ...
    """
    logging.info(
        "Found existing README.md with meaningful content. Will enhance rather than replace."
    )
    ...
    try:
        ...
        _ = await generate_overview(llm_client, file_manifest, project_name)
...
async def generate_overview(
    llm_client: BaseLLMClient, file_manifest: dict, project_name: str
) -> str:
    """
    ...
    try:
        ...
        if usage_text and len(usage_text) > CONTENT_THRESHOLDS["usage_text_length"]:
            ...
            if len(first_para) > CONTENT_THRESHOLDS["overview_paragraph_length"]:
                logging.info(
                    f"Using first paragraph of usage as overview, length: {len(first_para)}"
                )
                return first_para
    except Exception as e:
    ...
    if arch_path.exists():
        try:
            ...
            overview_match = re.search(
                r"## Overview\s+(.+?)(?=##|\Z)", arch_content, re.DOTALL
            )
            if overview_match:
                overview = overview_match.group(1).strip()
                logging.info(
                    f"Using overview from ARCHITECTURE.md, length: {len(overview)}"
                )
                return overview
        except Exception as e:
...
def add_architecture_link_if_needed(
    content: str, architecture_file_exists: bool
) -> str:
    """
...
def log_validation_issues(validation_issues: List[Any]) -> None:
    ...
    """
    link_issues = [
        issue for issue in validation_issues if "Link issue" in issue.message
    ]
    format_issues = [
        issue for issue in validation_issues if "Link issue" not in issue.message
    ]
