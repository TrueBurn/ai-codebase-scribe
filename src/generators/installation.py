...
CONTENT_THRESHOLDS = {
    "installation_guide_length": 50,  # Minimum length for a valid installation guide
    "readability_score_threshold": 40,  # Threshold for readability warnings
}


async def generate_installation(
    repo_path: Path,
    file_manifest: dict,
    ...
    """
    Generate INSTALLATION.md content.
    ...
    Returns:
        str: Generated INSTALLATION.md content
    """
    try:
        ...
        if _use_visual_logging:
            logger.info(
                f"Generating installation guide for: {logger.format_filename(project_name)}",
                emoji="package",
            )
        else:
            logging.info(f"Generating installation guide for: {project_name}")

        # Check if we should enhance existing INSTALLATION.md or create a new one
        if should_enhance_existing_installation(repo_path, config):
            logger = _get_logger()
            if _use_visual_logging:
                logger.info(
                    "Found existing INSTALLATION.md with meaningful content. Will enhance rather than replace.",
                    emoji="package",
                )
            else:
                logging.info(
            ...
            try:
                enhanced_content = await enhance_existing_installation(
                    repo_path,
                    llm_client,
                    file_manifest,
                    project_name,
                ...
                if _use_visual_logging:
                    logger.debug(
                        f"Caught timeout in generate_installation: {type(e).__name__}: {e}"
                    )
                else:
                    logging.debug(
                        f"Caught timeout in generate_installation: {type(e).__name__}: {e}"
                ...
                if _use_visual_logging:
                    logger.warning(
                        "INSTALLATION enhancement timed out. Using original content",
                        emoji="timer",
                    )
                else:
                    logging.warning(
                ...
                # Read and return the original INSTALLATION content
                installation_path = repo_path / "docs" / "INSTALLATION.md"
                ...
                if _use_visual_logging:
                    logger.warning(
                        f"INSTALLATION enhancement failed: {e}. Using original content",
                        emoji="warning",
                    )
                else:
                    logging.warning(
            ...
            if _use_visual_logging:
                logger.info(
                    "Enhancement failed, proceeding to generate new INSTALLATION.md",
                    emoji="package",
                )
            else:
                logging.info(
        ...
        # Generate new INSTALLATION.md from scratch
        return await generate_new_installation(
            repo_path,
            llm_client,
            file_manifest,
            project_name,
        ...
        logger = _get_logger()
        if _use_visual_logging:
            logger.error(f"Error generating INSTALLATION.md: {e}", emoji="error")
            ...
            logger.info(
                "Falling back to minimal INSTALLATION.md template", emoji="template"
            )
        else:
            logging.error(f"Error generating INSTALLATION.md: {e}")
            logging.error(f"Exception type: {type(e).__name__}")
            logging.error(f"Exception details: {str(e)}")
            if hasattr(e, "__traceback__"):
                logging.error(f"Traceback: {traceback.format_exc()}")
            logging.info("Falling back to minimal INSTALLATION.md template")
        return generate_fallback_installation(repo_path)
...
def extract_existing_content_from_sources(
    ...
    """
    Extract existing installation-related content from README.md or CONTRIBUTING.md.

    Args:
        repo_path: Path to the repository root
        section_names: List of section names to extract (e.g., ["Installation", "Setup"])
...
def should_enhance_existing_installation(repo_path: Path, config: ScribeConfig) -> bool:
    """
    Determine if we should enhance an existing INSTALLATION.md.
    ...
    Returns:
        bool: True if we should enhance existing INSTALLATION.md
    """
    # Check if we should preserve existing content
    preserve_existing = config.preserve_existing
    installation_path = repo_path / "docs" / "INSTALLATION.md"
    # Check if INSTALLATION.md exists and preserve_existing is True
    if not (preserve_existing and installation_path.exists()):
        return False

    try:
        existing_content = installation_path.read_text(encoding="utf-8")
        # Check if it's not just a placeholder or default INSTALLATION
        return len(existing_content.strip().split("\n")) > 5
    except Exception as e:
        logger = _get_logger()
        if _use_visual_logging:
            logger.error(f"Error reading existing INSTALLATION.md: {e}", emoji="error")
        else:
            logging.error(f"Error reading existing INSTALLATION.md: {e}")
        return False


async def enhance_existing_installation(
    repo_path: Path,
    llm_client: BaseLLMClient,
    file_manifest: dict,
    project_name: str,
    ...
    """
    Enhance an existing INSTALLATION.md file.
    ...
    Returns:
        str: Enhanced INSTALLATION.md content
    """
    logger = _get_logger()
    if _use_visual_logging:
        logger.info(
    ...
    try:
        ...
        try:
            ...
            if "timeout" in str(e).lower() or "TimeoutError" in str(type(e)):
                ...
                if _use_visual_logging:
                    logger.warning(
                        f"INSTALLATION enhancement timed out for large file ({file_size_kb:.1f}KB). Using original content.",
                        emoji="timer",
                    )
                else:
                    logging.warning(
        ...
        if changes["content_changed"]:
            if enhancement_log:
                ...
                if _use_visual_logging:
                    logger.success(
                        f"INSTALLATION.md enhanced with changes: {enhancement_log}",
                        emoji="package",
                    )
                else:
                    logging.info(
                ...
                if _use_visual_logging:
                    logger.success(
                        f"INSTALLATION.md enhanced (size change: {changes['size_change_bytes']} bytes)",
                        emoji="package",
                    )
                else:
                    logging.info(
            ...
            if _use_visual_logging:
                logger.info(
                    "INSTALLATION.md: No meaningful changes made by enhancement",
                    emoji="info",
                )
            else:
                logging.info(
        ...
        if _use_visual_logging:
            logger.error(
                f"Error enhancing existing INSTALLATION.md: {e}", emoji="error"
            )
            logger.error(f"Exception type: {type(e).__name__}", emoji="error")
            if hasattr(e, "__traceback__"):
                logger.error(f"Traceback: {traceback.format_exc()}", emoji="error")
            logger.info(
                "Falling back to generating new INSTALLATION.md", emoji="package"
            )
        else:
            logging.error(f"Error enhancing existing INSTALLATION.md: {e}")
            logging.error(f"Exception type: {type(e).__name__}")
            if hasattr(e, "__traceback__"):
                logging.error(f"Traceback: {traceback.format_exc()}")
            logging.info("Falling back to generating new INSTALLATION.md")
        # Return None to trigger fallback to new generation
        return None


async def progressive_enhance_installation(
    ...
    """
    Enhance INSTALLATION by processing sections individually to avoid timeouts.

    Args:
        existing_content: Current INSTALLATION content
        llm_client: LLM client for generating content
        file_manifest: Dictionary of files in the repository
        project_name: Name of the project
        related_repo_data: Optional related repository data for context
    ...
    Returns:
        str: Progressively enhanced INSTALLATION content
    """
    logger = _get_logger()
    if _use_visual_logging:
        logger.info(
            "Using progressive enhancement strategy for large INSTALLATION",
            emoji="package",
        )
    else:
        logging.info("Using progressive enhancement strategy for large INSTALLATION")

    try:
        # Split content into logical sections
        sections = split_installation_sections(existing_content)
        enhanced_sections = []
        ...
        for i, section in enumerate(sections):
            ...
            try:
                ...
                enhanced_section = await llm_client.enhance_documentation(
                    ...
                    file_manifest=file_manifest,
                    doc_type=f"INSTALLATION.md section: {section_name}",
                )
        ...
        if _use_visual_logging:
            logger.success(
                "Progressive INSTALLATION enhancement completed", emoji="package"
            )
        else:
            logging.info("Progressive INSTALLATION enhancement completed")
...
def split_installation_sections(content: str) -> List[Dict[str, str]]:
    """
    Split INSTALLATION content into logical sections based on headers.

    Args:
        content: INSTALLATION content to split
...
async def generate_new_installation(
    repo_path: Path,
    llm_client: BaseLLMClient,
    file_manifest: dict,
    project_name: str,
    ...
    """
    Generate a new INSTALLATION.md from scratch.
    ...
    installation = await generate_installation_content(
        llm_client,
        file_manifest,
        CONTENT_THRESHOLDS["installation_guide_length"],
        f"# {project_name} Installation\n\nPlease refer to the project documentation for installation instructions.",
        related_repo_data,
    )

    # Ensure the document has a proper title
    if not installation.strip().startswith("# "):
        installation = f"# {project_name} Installation\n\n{installation}"
    ...
    if related_repo_data:
        ...
        if related_repo_data.get("config") or related_repo_data.get("gitops"):
            remote_config_section += "Ensure you have access to these repositories before proceeding with installation."
            installation += remote_config_section

    # Validate and improve the content
    return await validate_and_improve_content(installation, repo_path)


async def generate_installation_content(
    llm_client: BaseLLMClient,
    file_manifest: dict,
    min_length: int,
    fallback_text: str,
    ...
    """
    Generate installation guide content with error handling.
    ...
    Returns:
        str: Generated installation guide content
    """
    try:
        # Call the LLM client's generate_installation_guide method
        content = await llm_client.generate_installation_guide(file_manifest)
        ...
        logger = _get_logger()
        if _use_visual_logging:
            logger.error(f"Error generating installation guide: {e}", emoji="error")
            logger.error(f"Exception type: {type(e)}", emoji="error")
            logger.error(
                f"Exception traceback: {traceback.format_exc()}", emoji="error"
            )
        else:
            logging.error(f"Error generating installation guide: {e}")
...
def ensure_correct_title(content: str, project_name: str) -> str:
    """
    Ensure the INSTALLATION.md has the correct project name in the title.

    Args:
        content: INSTALLATION.md content
        project_name: Name of the project
    ...
    if title_match:
        ...
        if "installation" not in old_title.lower() or "project" in old_title.lower():
            return content.replace(f"# {old_title}", f"# {project_name} Installation")
    else:
        # No title found, add one
        return f"# {project_name} Installation\n\n{content}"
...
async def validate_and_improve_content(content: str, repo_path: Path) -> str:
    ...
    Args:
        content: INSTALLATION.md content to validate
        repo_path: Path to the repository
...
def check_readability(content: str) -> None:
    ...
    score = scorer.analyze_text(content, "INSTALLATION")
    ...
    if isinstance(score, dict):
        ...
        if overall_score > threshold:  # Higher score means more complex text
            if hasattr(logger, "warning"):
                logger.warning(
                    "INSTALLATION.md content may be too complex.", emoji="warning"
                )
                logger.info(
                ...
                logging.warning("INSTALLATION.md content may be too complex.")
...
def generate_fallback_installation(repo_path: Path) -> str:
    """
    Generate a minimal valid INSTALLATION.md in case of errors.

    Args:
        repo_path: Path to the repository

    Returns:
        str: Minimal INSTALLATION.md content
    """
    return f"""# {repo_path.name} Installation

This guide provides instructions for installing and setting up {repo_path.name}.

## Prerequisites

Before installing {repo_path.name}, ensure you have the following:

- Operating system requirements (specify based on your project)
...
```bash
# Add package manager command (e.g., npm install, pip install -r requirements.txt)
```

### Step 3: Configuration

Configure the application by setting up environment variables and configuration files.
...
```bash
# Add build command if needed (e.g., npm run build, make)
```
...
```bash
# Add verification command (e.g., npm test, python -m pytest)
```
...
4. Consult the project documentation or issue tracker
...
---
*This INSTALLATION.md was automatically generated.*
"""
