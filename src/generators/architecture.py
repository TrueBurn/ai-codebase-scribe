...
from ..utils.tree_formatter import _format_tree_node, format_tree_structure
...
async def generate_architecture(
    ...
    try:
        ...
        try:
            ...
            architecture_content = await llm_client.generate_architecture_doc(
                file_manifest
            )
            ...
            if debug_mode:
                content_preview = (
                    architecture_content[:200] if architecture_content else "None"
                )
                logging.info(f"LLM response preview: {content_preview}...")
                logging.info(
                    f"Generated architecture content length: {len(architecture_content) if architecture_content else 0}"
                )
        ...
        if architecture_content and len(architecture_content) > MIN_CONTENT_LENGTH:
            ...
            try:
                ...
                if diagram and "```mermaid" in diagram:
                    ...
                    if (
                        "## Project Structure" in architecture_content
                        or "## 2. Project Structure" in architecture_content
                    ) and "```" in architecture_content:
                        ...
                        architecture_content = re.sub(
                            pattern, replacement, architecture_content, flags=re.DOTALL
                        )
            except Exception as e:
            ...
            logging.warning(
                f"Architecture content too short or invalid: {architecture_content[:100]}..."
            )
            return create_fallback_architecture(project_name, file_manifest)
    except Exception as e:
            ...
            if pattern in lower_component:
                for dep_pattern in dependencies:
                    ...
                    for potential_dep in list(graph.nodes()):
                        if (
                            dep_pattern in potential_dep.lower()
                            and potential_dep != component
                        ):
        ...
        if imports:
            for imported in imports:
                ...
                for potential_component in components:
                    if (
                        potential_component in imported
                        and component != potential_component
                    ):
        ...
        for component in components:
            ...
            if base_component in common_components:
                for dependency in common_components[base_component]:
                    for potential_component in components:
                        if (
                            dependency in potential_component
                            and component != potential_component
                        ):
                            graph.add_edge(component, potential_component)
