...
def should_enhance_existing_contributing(repo_path: Path, config: ScribeConfig) -> bool:
    ...
    )  # In root directory for GitHub recognition
...
async def enhance_existing_contributing(
    repo_path: Path, llm_client: BaseLLMClient, file_manifest: dict, project_name: str
) -> str:
    """
    ...
    """
    logging.info(
        "Found existing CONTRIBUTING.md with meaningful content. Will enhance rather than replace."
    )
...
async def generate_contributing_content(
    llm_client: BaseLLMClient, file_manifest: dict, min_length: int, fallback_text: str
) -> str:
    """
    ...
    if title_match:
        ...
        if "contributing" not in old_title.lower() or "project" in old_title.lower():
            return content.replace(
                f"# {old_title}", f"# Contributing to {project_name}"
            )
    else:
