...
class TestMessageManager(unittest.TestCase):
    ...
    def test_get_project_overview_messages(self):
        ...
        with self.assertRaises(ValueError):
            MessageManager.get_project_overview_messages(
                "", tech_report, template_content
            )
    ...
    def test_check_and_truncate_messages_under_limit(self):
        ...
        # Call the method
        result = MessageManager.check_and_truncate_messages(
            messages, mock_token_counter, "test-model"
        )

        # Verify the result
        self.assertEqual(result, messages)
        mock_token_counter.will_exceed_limit.assert_called_once_with(
            messages, "test-model"
        )
    ...
    def test_check_and_truncate_messages_intelligent_reduction(self):
        ...
        messages = [
            ...
            {
                "role": "user",
                "content": "Very long content that exceeds the token limit.",
            },
        ]
    ...
    def test_check_and_truncate_messages_hard_truncation(self):
        ...
        mock_token_counter.handle_oversized_input.return_value = (
            "Intelligently reduced but still too long"
        )
        mock_token_counter.count_message_tokens.return_value = (
            50  # System message tokens
        )
    ...
    def test_check_and_truncate_messages_invalid_inputs(self):
        ...
        with self.assertRaises(ValueError):
            MessageManager.check_and_truncate_messages(
                "not a list", mock_token_counter, "test-model"
            )
