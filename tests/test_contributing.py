...
from src.generators.contributing import (
    CONTENT_THRESHOLDS,
    check_readability,
    enhance_existing_contributing,
    ensure_correct_title,
    generate_contributing,
    generate_contributing_content,
    generate_fallback_contributing,
    generate_new_contributing,
    should_enhance_existing_contributing,
...
class TestContributingGenerator:
    ...
    def test_should_enhance_existing_contributing(self, tmp_path, test_config):
        ...
        assert (
            should_enhance_existing_contributing(repo_path, config_no_preserve) is False
        )
    ...
    async def test_generate_contributing_content(self, mock_llm_client):
        ...
        result = await generate_contributing_content(
            mock_llm_client, {}, 10, "Fallback text"
        )
        assert result == "Fallback text"
    ...
    async def test_generate_new_contributing(
        self,
    ...
    async def test_generate_contributing(
        self,
        mock_generate_new,
