...
from src.clients.bedrock import BEDROCK_API_VERSION, BedrockClient, BedrockClientError
...
async def test_initialization(bedrock_config):
    ...
    with patch("src.clients.bedrock.boto3.client") as mock_boto3_client, patch(
        "src.clients.bedrock.TokenCounter"
    ) as mock_token_counter, patch(
        "src.clients.bedrock.PromptTemplate"
    ) as mock_prompt_template, patch.dict(
        "os.environ", {"AWS_BEDROCK_MODEL_ID": ""}
...
async def test_validate_aws_credentials_success(bedrock_config):
    ...
    with patch("src.clients.bedrock.boto3.client") as mock_boto3_client, patch(
        "src.clients.bedrock.asyncio.to_thread"
    ) as mock_to_thread:
...
async def test_generate_usage_guide(bedrock_config):
    ...
    with patch("src.clients.bedrock.boto3.client") as mock_boto3_client, patch.object(
        BedrockClient, "_create_and_invoke_bedrock_request"
    ) as mock_invoke:
        ...
        with patch("src.clients.bedrock.MessageManager") as mock_message_manager:
            ...
            mock_message_manager.get_usage_guide_messages.assert_called_once_with(
                client.project_structure, "Test dependencies"
            )
...
async def test_get_file_order(bedrock_config):
    ...
    with patch("src.clients.bedrock.boto3.client") as mock_boto3_client, patch.object(
        ...
        "src.clients.bedrock.prepare_file_order_data"
    ) as mock_prepare_data, patch(
        "src.clients.bedrock.process_file_order_response"
        ...
        with patch("src.clients.bedrock.MessageManager") as mock_message_manager:
            ...
            # Verify MessageManager call
            mock_message_manager.get_file_order_messages.assert_called_once_with(
                "Files info"
            )
...
async def test_close(bedrock_config):
    ...
    with patch("src.clients.bedrock.boto3.client") as mock_boto3_client, patch(
        "src.clients.bedrock.asyncio.all_tasks"
    ) as mock_all_tasks, patch(
        "src.clients.bedrock.asyncio.current_task"
    ) as mock_current_task, patch(
        "src.clients.bedrock.asyncio.gather"
    ) as mock_gather, patch.dict(
        "os.environ", {"AWS_BEDROCK_MODEL_ID": ""}
    ):  # Clear environment variable
...
async def test_initialize(bedrock_config):
    ...
    with patch("src.clients.bedrock.boto3.client") as mock_boto3_client, patch(
        "src.clients.bedrock.TokenCounter"
    ) as mock_token_counter, patch.object(
        BedrockClient, "validate_aws_credentials"
    ) as mock_validate_credentials, patch.dict(
        "os.environ", {"AWS_BEDROCK_MODEL_ID": "", "AWS_ACCESS_KEY_ID": "test_key"}
    ):
...
async def test_generate_component_relationships(bedrock_config):
    ...
    with patch("src.clients.bedrock.boto3.client") as mock_boto3_client, patch.object(
        BedrockClient, "_invoke_model_with_token_management"
    ) as mock_invoke_model, patch.dict("os.environ", {"AWS_BEDROCK_MODEL_ID": ""}):
...
async def test_enhance_documentation(bedrock_config):
    ...
    with patch("src.clients.bedrock.boto3.client") as mock_boto3_client, patch.object(
        ...
        client._fix_markdown_issues = MagicMock(
            return_value="Fixed enhanced documentation"
        )
        ...
        # Test method
        result = await client.enhance_documentation(
            "Existing content", file_manifest, "README"
        )
        ...
        client.prompt_template.get_template.assert_called_once_with(
            "enhance_documentation",
            {"doc_type": "README", "existing_content": "Existing content"},
        )
...
async def test_invoke_model_with_token_management(bedrock_config):
    ...
    with patch("src.clients.bedrock.boto3.client") as mock_boto3_client, patch(
        "src.clients.bedrock.asyncio.to_thread"
    ) as _, patch("src.clients.bedrock.asyncio.wait_for") as mock_wait_for, patch.dict(
        "os.environ", {"AWS_BEDROCK_MODEL_ID": ""}
    ):
        ...
        client._prioritize_architectural_content = MagicMock(
            return_value="Truncated content"
        )
        client.token_counter.truncate_text = MagicMock(
            return_value="Further truncated content"
        )
