...
from src.utils.link_validator import LinkIssue, LinkValidator
...
"""
    ...
    async def test_validate_document(self):
        ...
        assert any(
            issue.message == "Referenced file does not exist" for issue in issues
        )
    ...
    async def test_validate_external_link_broken(self):
        ...
        with patch("httpx.AsyncClient", return_value=mock_client):
            await validator._validate_external_link(
                "https://nonexistent.example.com", 1
            )
    ...
    async def test_validate_external_link_timeout(self):
        ...
        mock_client = MagicMock()
        mock_client.__aenter__.return_value.head.side_effect = httpx.TimeoutException(
            "Timeout"
        )
    ...
    async def test_validate_external_link_retry_success(self):
        ...
        mock_client.__aenter__.return_value.head = MagicMock(
