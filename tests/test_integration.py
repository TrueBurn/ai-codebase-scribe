...
class TestIntegration:
    ...
    def config(self):
        """Test configuration."""
        from src.utils.config_class import BlacklistConfig, CacheConfig, ScribeConfig
        ...
        config.cache = CacheConfig(ttl=3600, max_size=1048576)
        return config
    ...
    def test_error_handling(self, temp_repo, config):
        ...
        with pytest.raises(ValueError, match="Repository path is not a directory"):
            _ = CodebaseAnalyzer(file_path, config)
        ...
        with pytest.raises(ValueError, match="Repository path does not exist"):
            _ = CodebaseAnalyzer(non_existent_path, config)
