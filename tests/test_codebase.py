...
def test_analyze_python_files(analyzer):
    ...
    python_files_in_manifest = {
        Path(path_str)
        for path_str, info in analyzer.file_manifest.items()
...
def test_derive_project_name_pom_xml(analyzer):
    ...
    # Test with pom.xml
    analyzer.file_manifest = {
        "pom.xml": create_file_info(
            "<project><name>TestProject</name><artifactId>test-artifact</artifactId></project>"
        )
    }
...
def test_derive_project_name_csharp(analyzer):
    ...
    assert (
        namespace_match.group(1) == "TestProject"
    ), f"Expected 'TestProject', got '{namespace_match.group(1)}'"
...
def test_derive_project_name_fallbacks(analyzer):
    ...
    analyzer.file_manifest = {"src/file.txt": create_file_info("")}
