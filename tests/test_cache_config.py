...
def sample_config_dict():
    ...
    from src.utils.config_class import CacheConfig, ScribeConfig
...
class TestCacheManager:
    ...
    def test_init_with_sample_config(self, sample_config, temp_repo_path):
        ...
        try:
            ...
            )  # Different from test_init_with_scribe_config
            assert cache_manager.debug is True
            assert cache_manager.cache_dir == temp_repo_path / ".test_cache"
        finally:
            # Ensure connections are closed
    ...
    def test_home_directory_cache(self, sample_config):
        ...
        with patch("pathlib.Path.home") as mock_home, patch("os.makedirs"), patch(
            "src.utils.cache.CacheManager._init_db"
        ):
            mock_home.return_value = Path("/mock/home")

            cache_manager = CacheManager(
                enabled=True, repo_identifier="test-repo", config=sample_config
            )

            try:
                assert (
                    cache_manager.cache_dir == Path("/mock/home") / ".test_global_cache"
                )
            finally:
                # Ensure connections are closed
                cache_manager.close()
