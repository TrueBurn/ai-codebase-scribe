...
def mock_llm_client():
    ...
    mock_client.generate_contributing_guide = AsyncMock(
        return_value="Contributing guide"
    )
    return mock_client
...
class TestReadmeGenerator:
    ...
    async def test_generate_readme_with_dict(
        self,
        mock_generate_new,
        temp_repo_path,
        mock_llm_client,
    ...
    @pytest.mark.asyncio
    @patch("src.generators.readme.generate_new_readme")
    async def test_generate_readme_with_scribe_config(
        self,
        mock_generate_new,
        temp_repo_path,
        mock_llm_client,
    ...
    def test_should_enhance_existing_readme_with_dict(
        self, temp_repo_path, sample_config_dict
    ):
    ...
    def test_should_enhance_existing_readme_with_scribe_config(
        self, temp_repo_path, sample_config
    ):
    ...
    @pytest.mark.asyncio
    async def test_generate_new_readme_with_dict(
        self, temp_repo_path, mock_llm_client, file_manifest, sample_config_dict
    ):
        """Test generate_new_readme with a dictionary config."""
        with patch(
            "src.generators.readme.extract_license_info"
        ) as mock_extract_license:
    ...
    @pytest.mark.asyncio
    async def test_generate_new_readme_with_scribe_config(
        self, temp_repo_path, mock_llm_client, file_manifest, sample_config
    ):
