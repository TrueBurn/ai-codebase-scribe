...
def test_load_custom_config(config_with_custom_values):
    ...
    assert (
        config_manager.config["bedrock"]["region"] == "us-east-1"
    ...
    assert (
        config_manager.config["bedrock"]["model_id"]
        == "us.anthropic.claude-sonnet-4-20250514-v1:0"
    )  # Default value from DEFAULT_CONFIG
    assert config_manager.config["cache"]["enabled"] is False
    ...
    assert (
        config_manager.config["ollama"]["base_url"]
        == DEFAULT_CONFIG["ollama"]["base_url"]
    )
    assert (
        config_manager.config["bedrock"]["max_tokens"]
        == DEFAULT_CONFIG["bedrock"]["max_tokens"]
    )
