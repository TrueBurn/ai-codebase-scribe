...
from src.utils.config_class import BedrockConfig, OllamaConfig, ScribeConfig
...
def test_register_client_type():
    ...
    class MockClient(BaseLLMClient):
        ...
        async def enhance_documentation(
            self, existing_content, file_manifest, doc_type
        ):
...
async def test_bedrock_fallback_to_ollama(config):
    ...
    with patch("src.clients.llm_factory.BedrockClient") as mock_bedrock_client, patch(
        "src.clients.llm_factory.OllamaClient"
    ) as mock_ollama_client:

        # Setup mocks
        mock_bedrock_instance = AsyncMock()
        mock_bedrock_instance.initialize.side_effect = Exception(
