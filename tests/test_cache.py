...
from src.utils.cache import CacheEntry, CacheManager, MemoryCache, SQLiteCache
...
def cache_config():
    ...
    config.cache = CacheConfig(
        directory=".cache", location="repo", hash_algorithm="md5"
    )
    return config
...
def cache_manager(tmp_path, cache_config):
    ...
    cm = CacheManager(repo_path=tmp_path, config=cache_config)
    return cm
