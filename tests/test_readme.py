...
from src.generators.readme import (
    CONTENT_THRESHOLDS,
    INSTRUCTION_PHRASES,
    _clean_section_headers,
    _format_anchor_link,
    add_architecture_link_if_needed,
    check_readability,
    enhance_existing_readme,
    ensure_correct_title,
    extract_overview_from_architecture,
    generate_fallback_readme,
    generate_new_readme,
    generate_overview,
    generate_overview_with_fallbacks,
    generate_readme,
    generate_section,
...
class TestReadmeGenerator:
    ...
    def mock_llm_client(self):
        ...
        mock_client.generate_project_overview = AsyncMock(
            return_value="This is a test project overview."
        )
        mock_client.enhance_documentation = AsyncMock(
            return_value="# Enhanced Content\n\nThis is enhanced content."
        )
        mock_client.generate_usage_guide = AsyncMock(
            return_value="This is a usage guide."
        )
        mock_client.generate_contributing_guide = AsyncMock(
            return_value="This is a contributing guide."
        )
        mock_client.generate_license_info = AsyncMock(
            return_value="This is license information."
        )
        return mock_client
    ...
    def mock_validator(self):
        ...
        mock_validator.fix_common_issues.return_value = (
    ...
    def test_format_anchor_link(self):
        ...
        assert (
            _format_anchor_link("Section.With/Special(Chars)")
            == "sectionwithspecialchars"
        )
    ...
    def test_should_enhance_existing_readme(self, tmp_path, test_config):
        ...
        meaningful_content = "\n".join(
            [
                f"Line {i}"
                for i in range(CONTENT_THRESHOLDS["meaningful_readme_lines"] + 5)
            ]
        )
    ...
    async def test_enhance_existing_readme(
        self, mock_generate_overview, tmp_path, mock_llm_client
    ):
        ...
        assert (
            mock_llm_client.enhance_documentation.call_args[1]["existing_content"]
            == readme_content
        )
        ...
        mock_llm_client.enhance_documentation.return_value = (
    ...
    async def test_generate_section(self, mock_llm_client):
        ...
        result = await generate_section(
            mock_llm_client, {}, "usage_guide", 10, "Fallback text"
        )
        assert result == "Fallback text"
    ...
    async def test_validate_and_improve_content(
        self, mock_check_readability, MockValidator, tmp_path
    ):
    ...
    async def test_generate_new_readme(
        self,
        mock_generate_section,
    ...
    async def test_generate_readme(
        ...
        mock_enhance_existing.return_value = (
