...
def test_custom_templates(temp_config_file):
    ...
    assert (
        template_manager.templates["custom_template"]
        == "This is a {custom_value} template"
    )

    # Check versioned template
    assert "versioned_template" in template_manager.templates
    assert (
        template_manager.templates["versioned_template"]
        == "This is a versioned {template}"
    )
...
def test_add_template():
    ...
    result = template_manager.add_template(
        "new_template", "This is a {new} template with {placeholders}", "1.0.0"
    )
    ...
    formatted = template_manager.get_template(
        "new_template", {"new": "brand new", "placeholders": "multiple values"}
    )
