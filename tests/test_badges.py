...
def test_generate_badges_with_missing_license_file(
    sample_file_manifest, sample_repo_path
):
...
def test_generate_badges_with_license_read_error(
    sample_file_manifest, sample_repo_path
):
...
def test_generate_badges_with_github_actions(sample_repo_path):
    ...
    file_manifest = {
        "src/main.py": FileInfo(path="src/main.py", language="python"),
        ".github/workflows/ci.yml": FileInfo(
...
def test_generate_badges_custom_style(sample_file_manifest, sample_repo_path):
    ...
    with patch("pathlib.Path.exists", return_value=False):
        result = generate_badges(
            sample_file_manifest, sample_repo_path, badge_style="flat"
        )
